<?php
	...
	{	
		hook( "form_main", "projectsOverviewToolbar", "", 1);
	...
	function projectsDelegate()
	{
		...
		hook( "head_tags", "projectsFormCss" );
...
HTML;
		...
		while ( $section= $manager->clerk->query_fetchArray( $getSections ) )
		{
			...
			foreach ( $projects as $id => $data )
			{
				if ( $data['section'] == $section['id'] )
				{
					$thumbnail= ( empty( $data['thumbnail'] ) == false ) ? '<img src="' . $manager->getSetting( "projects_path", 2 ) . $data['slug'] . '/' . $data['thumbnail'] . '" />' : "";
					$tabContent.= '
...
HTML;
	...
	function projectsOverviewToolbar()
	{
		...
		$tools= array(
						'<a href="#" onclick="newProject(); return false;">New Project</a>',
						'<a href="#" onclick="newSection(); return false;">New Section</a>'
		);
	...
	function projectsFormCss()
	{
		global $manager;
		
		echo $manager->office->style( SYSTEM_URL . "modules/projects/assets/styles.css" );
	}
	...
	function projectsFormJs()
	{
		...
		echo $manager->office->jsfile( SYSTEM_URL . "modules/projects/assets/jquery.multifile.js" );
		echo $manager->office->jsfile( SYSTEM_URL . "modules/projects/assets/handlers.js" );
	}
	...
	function editProjectForm( $allowed_file_types )
	{
		...
		$tools= array(
					'<a href="#" onclick="newGroup(\'top\'); return false;">New Group</a>',
					'<a href="#" onclick="addTextBlock(\'top\'); return false;">Add text block</a>'
		);
		...
		// $manager->form->add_input( "file", "Filedata", "Select the files you would like to upload to this project (<strong>Max " . str_replace( "M", "mb", ini_get( "upload_max_filesize" ) ) . "</strong> per file)" );
		$manager->form->add_input( "hidden", "asstPath", "", $manager->clerk->config('ASSISTANTS_PATH') );
		$manager->form->add_input( "hidden", "uploadPath", "", $paths['path'] );
		$manager->form->add_input( "hidden", "uploadUrl", "", $paths['url'] );
		...
		foreach ( $flow as $part )
		{
			if ( strstr( $part, "textblock") )
			{
				...
				$grouped.= '
					<div id="file_group-' . $fileID . '" class="fileGroup textBlock">
						<div class="controls">
							<ul>
								<li class="handle">
									Drag
			...
			if ( strstr( $part, "group" ) )
			{
				...
				foreach ( $projectFiles as $file => $data )
				{
					...
					if ( $data['filegroup'] == $groupNum )
					{
						if ( $data['type'] == "image" )
						{
							$thumbExt= substr( $data['file'], strrpos( $data['file'], '.' ) );
							$sysThumb= str_replace( $thumbExt, ".systhumb" . $thumbExt, $data['file'] );
							$thumb= '<img src="' . $paths['url'] . $project['slug'] . "/" . $sysThumb .'" alt="" />';
						}
						else
						{
		...
		$tools= array(
					'<a href="#" onclick="newGroup(\'bottom\'); return false;">New Group</a>',
					'<a href="#" onclick="addTextBlock(\'bottom\'); return false;">Add text block</a>'
		);
