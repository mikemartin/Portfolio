<?php
	...
	function listPages()
	{
		...
		while ( $data= $manager->clerk->query_fetchArray( $get ) )
		{
			...
			echo '<li class="item" id="page_' . $data['id'] . '"><div class="controls inline mini"><ul><li class="handle">Drag</li><li class="edit"><a href="' . $url . '">Edit</a></li></ul></div><a href="' . $url . '" class="title">' . $data['name'] . '</a></li>';
		}
		echo '</div>';
	}
	...
	function toolbar()
	{
		...
		$toolbar= new Toolbar(
			array(
				"tools"	=>	array('<a onclick="newPage();">New Page</a>')
			)
		);
	...
	function formJavascript()
	{
		...
		echo $manager->office->jsfile( SYSTEM_URL . 'modules/pages/assets/handlers.js' );
	}
